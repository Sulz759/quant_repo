...
namespace VContainer.Internal
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, InjectTypeInfo> Cache = new();

        [ThreadStatic] private static Stack<DependencyInfo> circularDependencyChecker;
        ...
        public static InjectTypeInfo Analyze(Type type)
        {
            ...
            var maxParameters = -1;
            foreach (var constructorInfo in typeInfo.GetConstructors(BindingFlags.DeclaredOnly | BindingFlags.Instance |
                ...
                if (constructorInfo.IsDefined(typeof(InjectAttribute), false))
                {
                    if (++annotatedConstructorCount > 1)
                        throw new VContainerException(type,
                            $"Type found multiple [Inject] marked constructors, type: {type.Name}");
            ...
            if (injectConstructor == null)
            {
                ...
                if (!allowNoConstructor)
                    throw new VContainerException(type,
                        $"Type does not found injectable constructor, type: {type.Name}");
            }
                ...
                foreach (var fieldInfo in fields)
                    if (fieldInfo.IsDefined(typeof(InjectAttribute), false))
                    {
                        ...
                        else
                        {
                            ...
                            if (injectFields.Any(x => x.Name == fieldInfo.Name)) continue;
                        }
        ...
        public static void CheckCircularDependency(IReadOnlyList<Registration> registrations, Registry registry)
        {
            ...
            for (var i = 0; i < registrations.Count; i++)
            {
                ...
                CheckCircularDependencyRecursive(new DependencyInfo(registrations[i]), registry,
                    circularDependencyChecker);
            }
        }

        private static void CheckCircularDependencyRecursive(DependencyInfo current, Registry registry,
            Stack<DependencyInfo> stack)
        {
            var i = 0;
            foreach (var dependency in stack)
            {
                if (current.ImplementationType == dependency.ImplementationType)
                {
                    ...
                    if (current.Dependency.Provider is FuncInstanceProvider) return;
                    ...
                    var path = string.Join("\n",
                        stack.Take(i + 1)
                            ...
                            .Select((item, itemIndex) =>
                                $"    [{itemIndex + 1}] {item} --> {item.ImplementationType.FullName}"));
            ...
            if (Cache.TryGetValue(current.ImplementationType, out var injectTypeInfo))
            {
                if (injectTypeInfo.InjectConstructor != null)
                    foreach (var x in injectTypeInfo.InjectConstructor.ParameterInfos)
                        if (registry.TryGet(x.ParameterType, out var parameterRegistration))
                            CheckCircularDependencyRecursive(
                                new DependencyInfo(parameterRegistration, current.Dependency,
                                    injectTypeInfo.InjectConstructor.ConstructorInfo, x), registry, stack);
                ...
                if (injectTypeInfo.InjectMethods != null)
                    ...
                    foreach (var x in methodInfo.ParameterInfos)
                        if (registry.TryGet(x.ParameterType, out var parameterRegistration))
                            CheckCircularDependencyRecursive(
                                new DependencyInfo(parameterRegistration, current.Dependency, methodInfo.MethodInfo, x),
                                registry, stack);
                ...
                if (injectTypeInfo.InjectFields != null)
                    foreach (var x in injectTypeInfo.InjectFields)
                        if (registry.TryGet(x.FieldType, out var fieldRegistration))
                            CheckCircularDependencyRecursive(
                                new DependencyInfo(fieldRegistration, current.Dependency, x), registry, stack);
                ...
                if (injectTypeInfo.InjectProperties != null)
                    foreach (var x in injectTypeInfo.InjectProperties)
                        if (registry.TryGet(x.PropertyType, out var propertyRegistration))
                            CheckCircularDependencyRecursive(
                                new DependencyInfo(propertyRegistration, current.Dependency, x), registry, stack);
            }
