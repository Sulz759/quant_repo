...
namespace _Project.Develop.Architecture.Runtime.Utilities
{
    public class SceneManager
    {
        ...
        public async UniTask LoadScene(int toLoadIndex)
        {
            ...
            var currentSceneIndex = UnitySceneManager.GetActiveScene().buildIndex;
            var isSkipEmpty = currentSceneIndex == RuntimeConstants.Scenes.Loading ||
                              currentSceneIndex == RuntimeConstants.Scenes.Bootstrap ||
                              toLoadIndex == currentSceneIndex;

            if (isSkipEmpty)
            {
                Log.Default.D(LogTag,
                    $"Empty scene skipped. {SceneUtility.GetScenePathByBuildIndex(toLoadIndex)} is loading.");
            ...
            var needLoadEmpty = toLoadIndex == RuntimeConstants.Scenes.Meta ||
            ...
            {
                Log.Default.D(LogTag,
                    $"{SceneUtility.GetScenePathByBuildIndex(RuntimeConstants.Scenes.Empty)} is loading.");
